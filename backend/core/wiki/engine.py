...
class LLMWikiEngine:
    ...
    def _persist_query_payload(self, payload: Dict[str, Any]) -> Optional[str]:
        ...
        retrieval_lines = [
            ...
            f"- context_max_chars: {effective_query_context_max_chars}",
    ...
    def _trim_incremental_updates_text(
        ...
        # Summarize overflow blocks via LLM into one condensed block
    ...
    def _rebuild_index_concise(self) -> None:
        ...
        Format: one line per page: ``[[path]] - title``
        This is designed to be passed as system context so the model can
        select which pages to read without needing lexical search.
        """
        lines = ["# Wiki Index (Entities & Concepts)", ""]

        for label, subdir in [("Entities", "entities"), ("Concepts", "concepts")]:
    ...
    def _import_graphify_module(self, module_name: str):
        try:
            ...
            graphify_root = Path(__file__).resolve().parents[3] / "graphify"
