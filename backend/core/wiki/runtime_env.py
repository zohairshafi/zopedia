...
WIKI_ENV_SPECS: tuple[WikiEnvSpec, ...] = (
    ...
    WikiEnvSpec(
        name="ZOPEDIA_ALPACA_PAPER",
        kind="bool",
        default="true",
        description="Trade against the Alpaca paper account (paper-api.alpaca.markets). Set false only when intentionally going live — see ZOPEDIA_ALPACA_ALLOW_LIVE.",
    ),
    WikiEnvSpec(
        name="ZOPEDIA_ALPACA_TRADING_BASE_URL",
        kind="string",
        default="",
        description="Override the Alpaca trading base URL entirely. Leave empty to derive it from ZOPEDIA_ALPACA_PAPER.",
    ),
    WikiEnvSpec(
        name="ZOPEDIA_ALPACA_ALLOW_LIVE",
        kind="bool",
        default="false",
        description="Safety gate: orders are refused when the trading base URL resolves to the live host unless this is explicitly true.",
    ),
    WikiEnvSpec(
        name="ZOPEDIA_ALPACA_APPROVAL_TTL_SECONDS",
        kind="int",
        default="86400",
        minimum=60,
        maximum=2592000,
        description="How long a research-proposed order stays approvable before it expires unplaced.",
    ),
    WikiEnvSpec(
        name="ZOPEDIA_ALPACA_APPROVAL_CHAT_TIMEOUT_SECONDS",
        kind="int",
        default="600",
        minimum=30,
        maximum=3600,
        description="How long a chat order-approval card waits for your decision before it expires unplaced.",
    ),
    WikiEnvSpec(
        name="ZOPEDIA_ALPACA_MAX_PENDING_PER_RUN",
        kind="int",
        default="5",
        minimum=1,
        maximum=50,
        description="Cap on orders a single headless research run may queue for approval.",
    ),
    WikiEnvSpec(
